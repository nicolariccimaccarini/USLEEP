...
CONFIG = {
    ...
    'overlap_ratio': 0.2,
    'num_clusters': 3, 
    'anomaly_percentile': 75,  # Percentile per soglia anomalia (distanza centroidi)
...
def compute_hybrid_anomaly_scores_with_amplitude(encoder, segments, segment_amplitudes):
    ...
    Returns:
        scores: array di punteggi anomalia normalizzati (0-1)
    ...
    # Calcola distanze dai centroidi (anomaly scores)
    distances = []
    ...
    # Normalizza i punteggi (distanza maggiore = più anomalo)
...
def process_channel_for_spindles(channel_name, data, sfreq, encoder):
    ...
    try:
        ...
        print(f"   🎯 K-Means clustering (3 cluster) completato")
        ...
        # Soglia basata su percentile degli anomaly scores
        ...
        for start_time, end_time in time_regions:
            ...
            if CONFIG['min_spindle_duration'] <= duration <= CONFIG['max_spindle_duration']:
                ...
                # Calcola statistiche della regione
                region_amplitudes = segment_amplitudes[start_seg:end_seg+1]
                region_anomaly_scores = anomaly_scores[start_seg:end_seg+1]
                region_clusters = cluster_labels[start_seg:end_seg+1]
        ...
        print(f"   📊 Anomaly scores - Media: {np.mean(anomaly_scores):.3f}, "
        ...
        return pd.DataFrame(columns=['Canale', 'Start_Time(s)', 'End_Time(s)', 'Duration(s)', 
                                    'Mean_Amplitude(µV)', 'Mean_Anomaly_Score', 'Dominant_Cluster'])
...
def main():
    ...
    cluster_output_path = os.path.join(dirData, "cluster_hybrid")
    
    os.makedirs(cluster_output_path, exist_ok=True)
    
    print(f"\n🔬 Modalità: Clustering Ibrido (3 Cluster + Distanze + Ampiezza)")
    print(f"📊 Numero cluster: {CONFIG['num_clusters']}")
    print(f"📊 Soglia anomalia: percentile {CONFIG['anomaly_percentile']}")
    ...
    if all_spindle_results:
        ...
        # Salva CSV
        csv_path = os.path.join(cluster_output_path, 'spindles_hybrid_clustering.csv')
        ...
        print(f"📊 Anomaly score medio: {final_results['Mean_Anomaly_Score'].mean():.3f}")
        
        print("\n📊 Distribuzione cluster dominanti:")
        print(final_results['Dominant_Cluster'].value_counts().sort_index())
